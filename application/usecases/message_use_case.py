...
class MessageUseCase(MessageUseCaseService):
    def __init__(self,
                 message_repository_service: MessageRepositoryService,
        ...
        self._message_repository_service = message_repository_service
    ...
    def send_message(self, user_id: int, message_data: Dict) -> Tuple:
        ...
        user_receive = self._user_repository_service.find_by_username(name=message_data.get('send_to'))
        if not user_receive:
    ...
    def read_message(self, user_id: int, message_id: int) -> Tuple:
        message = self._message_repository_service.find_msg_by_id(id=message_id)
        user_send = self._user_repository_service.find_by_id(message.from_user)
        user_receive = self._user_repository_service.find_by_id(message.to_user)
        ...
        if user_id == user_send.id:
            self._log_use_case_service.high_priority_log(id=user_id, msg=f"Read Msg to User: [{user_receive.id}]")
            return self._return_message(message=message, update_read_status=False), 200

        if user_id == user_receive.id:
            self._log_use_case_service.high_priority_log(id=user_id,
                                                         msg=f"Read Msg from User: [{user_send.id}], Mark as READ")
    ...
    def get_all_unread_messages(self, user_id: int) -> Tuple:
        all_messages = self._message_repository_service.find_all_unread(id=user_id)
        self._log_use_case_service.low_priority_log(id=user_id, msg="Display all Unread messages titles")
    ...
    def get_all_read_messages(self, user_id: int) -> Tuple:
        all_messages = self._message_repository_service.find_all_read(id=user_id)
        self._log_use_case_service.low_priority_log(id=user_id, msg="Display all Read messages titles")
        all_messages_list = self._convert_messages_to_list(messages=all_messages)
        return all_messages_list, 204

    def get_all_received_messages(self, user_id: int) -> Tuple:
        all_messages = self._message_repository_service.find_all_received(id=user_id)
        self._log_use_case_service.low_priority_log(id=user_id, msg="Display all Received messages titles")
        all_messages_list = self._convert_messages_to_list(messages=all_messages)
        return all_messages_list, 204

    def get_all_sent_messages(self, user_id: int) -> Tuple:
        all_messages = self._message_repository_service.find_all_sent(id=user_id)
        self._log_use_case_service.low_priority_log(id=user_id, msg="Display all Sent messages titles")
        all_messages_list = self._convert_messages_to_list(messages=all_messages)
        return all_messages_list, 204

    def get_all_read_by_receiver_messages(self, user_id: int) -> Tuple:
        all_messages = self._message_repository_service.find_sent_n_read(id=user_id)
        self._log_use_case_service.low_priority_log(id=user_id, msg="Display all Sent messages titles")
        all_messages_list = self._convert_messages_to_list(messages=all_messages)
        return all_messages_list, 204
    ...
    def _convert_message_to_dict(self, message: "MessageSchema") -> Dict:
        ...
        if not is_message_checksum_valid:
            self._log_use_case_service.high_priority_log(id=message.from_user,
                                                         msg=f'Message {message.id} Been TAMPERED')
            return {message.id: 'Message Been TAMPERED'}
        aggregated_message = self._aggregate_message_to_dict(message=message)
        self._log_use_case_service.low_priority_log(id=message.id, msg=f"{message.id} MESSAGE CHECKSUM: TRUE")
        return aggregated_message

    def _aggregate_message_to_dict(self, message) -> Dict:
        read_status = "Read" if message.read_status else "Unread"
        read_at = self._message_repository_service.convert_timestamp(message.read_at) if message.read_at else "Unread"
        create_date = self._message_repository_service.convert_timestamp(message.create_date)
    ...
    def _compare_hash_keys(self, message) -> bool:
        hash_key = self._message_repository_service.generate_message_hash_key(user_send=message.from_user,
    ...
    def _send_message(self, user_send: int, user_receive: "UserRepository", message_data: Dict) -> None:
        ...
        self._message_repository_service.save_message(message_data=message_dict_model.dict())
        self._log_use_case_service.low_priority_log(id=user_send, msg=f"Msg Sent to User: [{user_receive.id}]")
    ...
    def _aggregate_message(self, user_send: int, user_receive: "UserRepository", message_data: Dict) -> Dict:
        message_schema = {"from_user": user_send,
                          ...
                          "create_date": self._message_repository_service.insert_timestamp(),
                          "hash": None}
    ...
    def _generate_message_hash_key(self, message: Dict) -> str:
        hash_key = self._message_repository_service.generate_message_hash_key(user_send=message.get('from_user'),
    ...
    def _return_message(self, message: "MessageRepository", update_read_status: bool) -> Dict:
        """Json MSG TITLE + BODY"""
        self._message_repository_service._update_read_status(id=message.id) if update_read_status else None
        message_read_status = "Read" if message.read_status else "Unread"
        message_read_at = self._message_repository_service.convert_timestamp(
            message.read_at) if message.read_at else "Unread"
